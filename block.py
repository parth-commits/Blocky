...
class Block:
    ...
    def _update_children_positions(self, position: Tuple[int, int]) -> None:
        ...
        Block.
        """
        # do we only update the first children, or grandchildren too?
        # ie, is this recursive or not? THIS IS RECURSIVE
        if self.children:
            self.position = position
            temp = self._children_positions()
            for x in range(4):
                self.children[x].position = temp[x]
                self.children[x]._update_children_positions(temp[x])
    ...
    def smash(self) -> bool:
        ...
        level = self.level + 1
        ul = Block((0, 0), new_size, None, level, max_depth)
        ur = Block((0, 0), new_size, None, level, max_depth)
        ll = Block((0, 0), new_size, None, level, max_depth)
        lr = Block((0, 0), new_size, None, level, max_depth)
        self.children = [ur, ul, ll, lr]
        self._update_children_positions(self.position)
        self.colour = None

        for child in self.children:
            ran = random.random()
            if ran < math.exp(-0.25 * self.level):
                if not child.smash():
    ...
    def swap(self, direction: int) -> bool:
        ...
        """
        # swap first child or grand children too?
        # is it recursive?
        if len(self.children) == 0:
            return False
        if direction == 1:
            ...
            self.children[0], self.children[1] = self.children[1], \
                                                 self.children[0]
            self.children[3], self.children[2] = self.children[2], \
                                                 self.children[3]
        self._update_children_positions(self.position)
        return True
    ...
    def rotate(self, direction: int) -> bool:
        ...
        if len(self.children) == 0:
            return False
        if direction == 1:
            temp = self.children.pop(0)
            self.children.append(temp)
            for x in range(4):
                self.children[x].rotate(direction)
        else:
            temp = self.children.pop()
            self.children.insert(0, temp)
            for x in range(4):
                self.children[x].rotate(direction)
        self._update_children_positions(self.position)
        return True
    ...
    def paint(self, colour: Tuple[int, int, int]) -> bool:
        ...
        """
        if self.level == self.max_depth and self.colour != colour and \
                not self.children:
            self.colour = colour
            return True
        return False
    ...
    def combine(self) -> bool:
        ...
        stuff = {}
        for child in self.children:
            if child.colour not in stuff:
                stuff[child.colour] = 1
            else:
                stuff[child.colour] += 1
        high = 0
        high_item = None
    ...
    def create_copy(self) -> Block:
        ...
        if self.children:
            for kid in self.children:
                child.append(kid.create_copy())
        copy = Block(pos, size, colour, level, max_depth)
        copy.children = child
        return copy
